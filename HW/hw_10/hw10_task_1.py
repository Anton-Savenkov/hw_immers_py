...
в зависимости от веса. Если вес объекта меньше 1, то он относится к категории
"Малявка".
...
животных разных типов на основе переданного типа и параметров. Класс-фабрика
должен иметь метод create_animal, который принимает следующие аргументы:
...
конкретного типа животного. Количество и типы аргументов могут различаться
...
# создадим класс-фабрику AnimalFactory:
class AnimalFactory:
    @staticmethod
    def create_animal(animal_type, *args):
        if animal_type == 'Bird':
            if len(args) != 2:
                raise ValueError("Недопустимые аргументы для экземпляра 'Bird'")
            return Bird(*args)
            ...
            if len(args) != 2:
                raise ValueError("Недопустимые аргументы для экземпляра 'Fish'")
            return Mammal(*args)
        else:
            raise ValueError("Недопустимый тип животного")
#проверенное решение выше
'''

# Создаем экземпляры животных с помощью класса-фабрики
bird_instance = AnimalFactory.create_animal('Bird', 'Eagle', 150)
fish_instance = AnimalFactory.create_animal('Fish', 'Salmon', 20)
mammal_instance = AnimalFactory.create_animal('Mammal', 'Elephant', 500)

# Проверяем результаты
print(bird_instance.wing_length())  # Выводит длину крыла птицы
print(fish_instance.depth())  # Выводит категорию глубины рыбы
print(mammal_instance.category())  # Выводит категорию млекопитающего
'''
