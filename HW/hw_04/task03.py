...
и вывод итоговой информации о состоянии счета и проведенных операциях.
...
Перед завершением, если на счету больше RICHNESS_SUM, начисляется налог
...
def check_multiplicity(amount):
    if amount % MULTIPLICITY == 0:
        return amount % MULTIPLICITY == decimal.Decimal('0')
    else:
...
def withdraw(amount):
    global balance
    removal = int(amount * PERCENT_REMOVAL)
    if removal < MIN_REMOVAL:
        removal = MIN_REMOVAL
    elif removal > MAX_REMOVAL:
        removal = MAX_REMOVAL

    if check_multiplicity(amount) and amount + removal <= balance:
        balance -= amount + removal
        operations.append(f'Снятие с карты {amount} у.е. Процент за снятие '
                          f'{removal} у.е. Итого {balance} у.е.')
    else:
        operations.append(f'Недостаточно средств. Сумма с комиссией '
              f'{amount + removal} у.е. На карте {balance} у.е.')
...
def exit():
    ...
    # Вывод остальной информации о проведенных операциях
